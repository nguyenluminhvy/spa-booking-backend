...
export class MailService {
  ...
  async sendResetPasswordByAdmin(email: string) {
    const html = `
    <!DOCTYPE html>
    <html>
    <head>
  ...
  async sendStaffInvitation(email: string, password: string, name?: string) {
    const html = `
        <!DOCTYPE html>
        <html>
        <head>
            <meta charset="UTF-8" />
            <title>Welcome to Spa Booking Y</title>
        </head>
        <body style="margin:0; padding:0; background-color:#f4f6f8; font-family:Arial, sans-serif;">
            ...
            <td align="center">
                ...
                <tr>
                    ...
                    <p style="color:#555; font-size:16px; margin:10px 0;">
                        You have been invited to join the Spa Booking system as a staff member.
                    </p>
    ...
    await this.transporter.sendMail({
      from: process.env.MAIL_FROM,
      to: email,
      subject: 'You are invited to join Spa Booking',
      html,
    });
  }
}
